...
typedef struct large_number { // Special Struct for Large Numbers 

    long array_length, current_array_length, free1, free2;
    char *n;
} lnumber;

struct Message { // Struct for pthread usage
    int myid, steps;
    lnumber x, y, result; 
};

typedef struct aux_div { // Auxiliar Struct used in Large Long Division
    lnumber sum;
    char amount;
    int q_is_zero; 
} aux_div;
...
int main(int argc, char **argv) {
    ...
    if ( argc == 2 && atoi(argv[1]) > 1) {
        ...
        for ( i = 0; i < p; i = i + 1 ) {
            ...
            if (i == 0 ) {
                ...
                if (i + 1 < p ){
                    ...
                    acum_pow2 = LargeTwoExpBySquaring(m[i]->steps);
                }
                ...
                m[i]->y = LargeRussianMult2(y_aux, acum_pow2, 1, 0); // cubierto
            ...
            if (i + 1 < p && last_steps != m[i]->steps){
                ...
                last_steps = m[i]->steps; 
                acum_pow2 = LargeDivByTwo(acum_pow2); 
            }

            pthread_create(&thread[i], &attribute, PLargeRussianMult, (void *) m[i]); // create proccess i
        }
        ...
        for ( i = 0; i < p; i = i + 1 ) {
            ...
            pthread_join(thread[i], &exit_status); // get back the partial sums results from the i-th thread 
...
lnumber LargeRussianMult1(lnumber n1, lnumber n2, int free1, int free2) {
    ...
    lnumber result, n2_new, n1_new; 

    result = InitLargeNumber("0", n1.array_length + n2.array_length, 1, 1);

    n2_new = InitLargeNumber(n2.n, n1.array_length + n2.array_length, n2.current_array_length, n2.array_length);
    n1_new = InitLargeNumber(n1.n, n1.array_length, n1.current_array_length, n1.array_length);

    while ( n1_new.current_array_length > 0 ){
        if ((n1_new.n[n1_new.array_length-1] - '0') % 2 != 0)
            result = LargeSum12(result, n2_new, result);
        
        n2_new = LargeSum12(n2_new, n2_new, n2_new);

        n1_new = LargeDivByTwo(n1_new);
    }
...
lnumber LargeLongDivision(lnumber n1, lnumber n2){
    ...
    while (n1.array_length > index_n1) {
        ...
        if (aux.q_is_zero == 0)
            aux.sum = LargeDiff11(aux.sum, n2, 'n', 0, 0);
